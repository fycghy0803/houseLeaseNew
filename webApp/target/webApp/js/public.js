...
function transferMes(msg, resObj) {
    ...
    if (msg) {
         var r = /\$\{((\w|\.)*)\}/g;
         var tmpRes,tmpVal;
         var group = msg.match(r);
         console.log(group);
         while( tmpRes = r.exec(msg)) {
           tmpVal = undefined;
           try {
             eval("tmpVal = resObj." + tmpRes[1]);
           } catch(e){}

           if(tmpVal != undefined) {
               if(tmpVal.replace) {
                 tmpVal = tmpVal.replace(" 00:00:00","");
               }
               tmpMsg = tmpMsg.replace(tmpRes[0],tmpVal);
            }else {
               tmpMsg = tmpMsg.replace(tmpRes[0],'');
            }
         }
...
function transferInputs(resObj) {
    var tmpVal;
    $(":input").each(function(a,inputObj){
        var name = $(inputObj).attr("name");
        if(name) {
          tmpVal = undefined;
          try {
            eval("tmpVal = resObj." + name);
          } catch(e){}
...
function validateNull(formId) {
  var form = $("#" + formId);
  var ret = true;
  $.each(form.find("input"),function(i,inputObj){
